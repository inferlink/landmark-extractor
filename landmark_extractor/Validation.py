...
class ValidationRule:
    ...
    def execute_rule(self, field_extraction):
        ...
        return self.__method(self, field_extraction, self.__param)
...
class Validation:
    ...
    # {"valid":false}
    @staticmethod
    def get_validation_for_page(page_extraction, validation):
        ...
        if isinstance(page_extraction, list):
            ...
            for row in page_extraction:
                ...
                if 'sub_rules' in row:
                    #print "in subrules...."
                    Validation.get_validation_for_page(row['sub_rules'], validation)
        elif isinstance(page_extraction, dict):
            #print 'dict ...'
            # this is a standard rule so we look at this one itself
            for name in page_extraction:
                ...
                if 'sub_rules' in page_extraction[name]:
                    #print "in subrules...."
                    Validation.get_validation_for_page(page_extraction[name]['sub_rules'], validation)
                elif 'sequence' in page_extraction[name]:
                    #print "in sequence ..."
                    Validation.get_validation_for_page(page_extraction[name]['sequence'], validation)
    ...
    def get_validation_for_extraction(extraction):
        ...
        for page in extraction:
            Validation.get_validation_for_page(extraction[page], validation)
            if validation['valid'] is False:
                return validation
        return validation
...
def main(argv=None):
    ...
    # print "validation:", v
    #
    # with codecs.open("page_extraction_1.json", "r", "utf-8") as myfile:
